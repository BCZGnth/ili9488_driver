...
void ili9488_draw_rect(Ili9488Defines * screen, Ili9488Rect * Rect){
    ...
    uint16_t hlength = (Rect->xend - Rect->xstart);
    // uint8_t hxstart = Rect->xstart + Rect->weight;
    uint16_t vlength = (Rect->yend - Rect->ystart);
...
/**
 * Simply draws a bar moving across the bottom of the screen. Every time the function is called,
 * another step in the bar is drawn.
 * @param screen, The Ili9488Defines structure used to define the screen API to use.
 */
