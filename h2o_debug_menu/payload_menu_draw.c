 ...
 * @param count, number of buttons (menu items) in menu
 * 
 * Draws a menu with a title, menu items, and an exit button. The limitation here is that we can not render
 * more than 4 menu items at a time. The logic for more is not available right now.
 */
void draw_menu_buttons(Ili9488Defines* screen, const char * title, Button * buttons, uint8_t count) {
    /* Draw the Frame */
    Ili9488Rect outline = {
        .xstart = 0,
        .ystart = 0,
    ...
    for(uint8_t i = 0; i < count; i++) {
        /* Draw button outline */
        button_rect.xstart = buttons[i].tb.xs;
        button_rect.xend   = buttons[i].tb.xe;
        button_rect.ystart = buttons[i].tb.ys;
        button_rect.yend   = buttons[i].tb.ye;
        ili9488_draw_rect(screen, &button_rect);

        /* Calculate text centering */
        uint8_t text_length = strlen(buttons[i].label);
        uint16_t text_width = text_length * screen->Screen.offset_2x.width_pad;
        uint16_t button_width = buttons[i].tb.xe - buttons[i].tb.xs;
        uint16_t button_height = buttons[i].tb.ye - buttons[i].tb.ys;

        uint16_t text_x_center, text_y_center;
        if(button_width > text_width) {
            text_x_center = buttons[i].tb.xs + (button_width / 2) - (text_width / 2);
        } else {
            text_x_center = buttons[i].tb.xs;
        }
        if(button_height > screen->Screen.offset_2x.height) {
            text_y_center = buttons[i].tb.ys + (button_height / 2) - (screen->Screen.offset_2x.height / 2);
        } else {
            text_y_center = buttons[i].tb.ys;
        }

        /* Draw button text */
        button_label.text = (char*)buttons[i].label;
        ...
        // button_label.ram_ptr.start_x = buttons[i].tb.xs;
        // button_label.ram_ptr.end_x   = buttons[i].tb.xe;
        // button_label.ram_ptr.start_y = buttons[i].tb.ys;
        // button_label.ram_ptr.end_y   = buttons[i].tb.ye;
        
        ili9488_print(screen, &button_label);
    }

    /* Draw title in whitelist area */
    Ili9488Print menu_name = {
        .text = title,
        .fg = YELLOW,
        .font = screen->Screen.offset_2x,
        .clear_before = 0,
        .ram_ptr = {
            ...
            .start_y = 10,
            .end_x = 309,
            .end_y = 10 + screen->Screen.offset_2x.height,
        },
    };
    ili9488_print(screen, &menu_name);
}
